...
//if(debug_> 0) LogPrint(MOD) << "Code732";
//tofCalculator.loadTimeOffset(timeOffset_); 
//if(debug_> 0) LogPrint(MOD) << "Code733";
}
...
void Analyzer::beginJob() {
  ...
  if (sampleType_ == 0 ) {
    if (debug_> 0) edm::LogPrint(MOD) << "This is data processing code731";
    //if (debug_> 0) edm::LogPrint(MOD) << "woxiangremix"; 
...
void Analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (currentRun_ != iEvent.id().run()) {
    ...
    //LogPrint(MOD) << "Code734";
    tofCalculator.setRun(currentRun_);
    //LogPrint(MOD) << "Code735";
    dEdxSF[0] = dEdxSF_0_;
    dEdxSF[1] = dEdxSF_1_;
  }
